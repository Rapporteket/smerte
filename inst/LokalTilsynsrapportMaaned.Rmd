...
if (rapbase::isRapContext()) {
  ...
  aar <- params$year
  dat = dat %>% filter(year(date(StartdatoTO)) == aar) #year #denne virker? Er ikke alle registert på regdato?
...
tilsyn_full = dat %>% select(AntPasTils, AntTilsLege, AntTilsSykPleier, AntTilsFysioT, AntTilsPsyk, AntTilsSosio, StartdatoTO, PasientID, ForlopsID, Tilsett, DEPARTMENT_NAME, DEPARTMENT_SHORTNAME, InnlAvd) %>% 
...
tilsyn_full = tilsyn_full %>% mutate(monthname = factor(months(as.Date(StartdatoTO)), levels = manednamn)) 
