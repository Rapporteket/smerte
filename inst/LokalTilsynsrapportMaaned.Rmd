...
if (context %in% c("DEV", "TEST", "QA", "PRODUCTION")) {
  ...
  avd <- avd %>% rename(DEPARTMENT_ID = AvdID, DEPARTMENT_NAME = AvdNavn)
...
tilsyn_full = dat %>% select(AntPasTils, AntTilsLege, AntTilsSykPleier, AntTilsFysioT, AntTilsPsyk, AntTilsSosio, RegDato11, PasientID, ForlopsID, Tilsett, DEPARTMENT_NAME, InnlAvd) %>% 
...
sum_tilsyn_avdeling = tilsyn_full %>% 
                     group_by(tilsynstype, DEPARTMENT_NAME) %>% 
                     summarise(antall = sum(antall)) %>% 
                     spread(tilsynstype, antall) %>% rowwise() %>% 
                     mutate(behandlertilsyn = sum(Fysioterapeut, Lege, Psykolog, Sosionom, Sykepleier))

#antall_pas_avdeling = tilsyn %>% distinct(pasientid, .keep_all = TRUE) %>% count(avdeling_navn) 
antall_pas_avdeling = tilsyn_full %>% distinct(ForlopsID, .keep_all = TRUE) %>% count(DEPARTMENT_NAME) 
...
sum_tilsyn_avdeling$DEPARTMENT_NAME = as.character(sum_tilsyn_avdeling$DEPARTMENT_NAME)

#Ta vekk de som ikke har hatt pasienter (f.eks andre sykehus sine avdelinger)
sum_tilsyn_avdeling <- sum_tilsyn_avdeling[!is.na(sum_tilsyn_avdeling$DEPARTMENT_NAME), ]
...
sum_tilsyn_avdeling = sum_tilsyn_avdeling %>% mutate(DEPARTMENT_NAME = str_replace_all(DEPARTMENT_NAME, "\n", " "))

mst(tab = sum_tilsyn_avdeling,
    colnames = c("Avdeling", "Fysio", "Lege", "Psykolog", "Sosionom", "Sykepleier", "Beh tilsyn", "Pas tilsyn", "Ant pas"),
    type = params$tableFormat,
