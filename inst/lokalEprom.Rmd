...
if (rapbase::isRapContext()) {
  dat <- smerte::getDataDump(
    registryName = params$registryName,
    reshId = params$reshId,
    tableName = if (smerte::isNationalReg(params$reshId)) "allevarnumnasjonal" else "allevarnum",
...
pasreg <- datpr %>% 
  count(GrManglUtfylPR) %>%
  mutate(prosent = round(100*(n/sum(n))))
#Gi navn i stedet for tall til alternativene for tilsyn/ikke tilsyn
pasreg = pasreg %>%
  mutate(grunnPR = case_when(GrManglUtfylPR == 1 ~ "Tidsmangel behandler",
                             GrManglUtfylPR == 2 ~ "Tidlig utskrivning",
                             GrManglUtfylPR == 3 ~ "Kritisk syk/død",
                             GrManglUtfylPR == 4 ~ "Pasient klarer ikke",
                             GrManglUtfylPR == 5 ~ "Manglende samtykke",
                             GrManglUtfylPR == 7 ~ "Ikke besvart ePPROMS",
                             GrManglUtfylPR == 9 ~ "Annet"))
...
andsvar <- NA
if(antUtsendt != 0){
  andsvar <- (antsvar/antUtsendt)*100
}else{andsvar <- paste0("ikke mulig å angi andel grunnet ingen utsendte skjema.")}

#Fjerner først de som ikke har fått tilsendt (null) 3 = null
datE <- dat[!is.na(dat$PasdelUtfylEval), ]

#De må også har vært tilsett (1)
datE <- datE[datE$Tilsett == 1, ]
...
#Lager ny datE som kun tar med de som har svart i prosentandelen

datE <- datE %>% dplyr::filter(PasdelUtfylEval == 1)
...
```
I den valgte tidsperioden fikk tilsammen `r antUtsendt` pasienter av totalt `r dim(dat)[1]` tilsendt ePROM-skjema for Eval. Av de som fikk tilsendt skjema var det `r antsvar` pasienter som sendte inn en besvarelse. Dette gir en svarprosent på `r round(andsvar, digits = 1)`. Tabell \@ref(tab:evalspmtab) gir en oversikt over registrerte årsaker til manglende besvarelse for de `r antiksvar` som ikke har svart.
...
#Lager datasett kun for de som faktisk har svart på skjema
datO <- datO %>% dplyr::filter(PasdelUtfylOpioid == 1)

#Kun de som faktisk er tilsett fra nå av
datO <- datO %>% dplyr::filter(Tilsett == 1)

```
I den valgte tidsperioden fikk tilsammen `r antUtsendt` pasienter av totalt `r dim(dat)[1]` tilsendt ePROM-skjema for Opioid. Av de som fikk tilsendt skjema var det `r antsvar` pasienter som sendte inn en besvarelse. Dette gir en svarprosent på `r round(andsvar, digits = 1)`. Tabell \@ref(tab:opiospmtab) gir en oversikt over registrerte årsaker til manglende besvarelse for de `r antiksvar` som ikke har svart.
...
#Lager ny df med kun de som har svar
