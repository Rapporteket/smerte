...
if (dim(dat)[1] < 1) {
  ...
  dat_aks = dat |> filter(Tilsett == 1)
  
  #Verdier som skal vises i tabell for denne indikatoren
  ant <- sum(dat_aks$AkseptabelSmerte21 == 1, na.rm = TRUE)
  
  totant <- sum(!is.na(dat_aks$AkseptabelSmerte21))
  
  and <- ant/totant
  
  #Før, for sammenligning men ikke indikator
  antfoer <- sum(dat_aks$AkseptabelSmerte12 == 1, na.rm = TRUE)
  
  totantfoer <- sum(!is.na(dat_aks$AkseptabelSmerte12))
