...
Andel pasienter hvor funksjon bedrer seg fra første til siste tilsyn.
...
if (dim(dat)[1] < 1) {
  ...
  datfunk <- dat %>% mutate(bedring = 
                              case_when((Funksjon12 | Funksjon21) == 9 ~ 9,
                                        sum(Funksjon21-Funksjon12, na.rm = TRUE) > 0 ~ 1,
                                        sum(Funksjon21-Funksjon12, na.rm = TRUE) < 0 ~ 0))
  
  ant <- sum(datfunk$bedring == 1, na.rm = TRUE)
  
  totant <- sum(!is.na(datfunk$bedring))
