...
Andel forløp hvor pasientene tilses av to eller flere faggrupper. Det er ønskelig med høyest mulig andel. De ulike faggruppene er lege, sykepleier, sosionom, psykolog, fysioterapeut eller om det er konferert med lege. Med "totalt antall" menes her alle forløp som har minimum ett tilsyn. 
...
if (dim(dat)[1] < 1) {
  ...
  d_ki_behandlertilsyn = ki_behandlertilsyn(dat) 
   
  d_ki_behandlertilsyn_per_sykehus = d_ki_behandlertilsyn |> 
    group_by(SykehusNavn) |> 
    aggreger_ki_prop() |> 
    select(-konfint_nedre, -konfint_ovre) |> 
    relocate(SykehusNavn, ki_teller, ki_nevner, est)
  
  #Tabell
  mst(
    tab = d_ki_behandlertilsyn_per_sykehus,
...
if (dim(dat)[1] < 1) {
  ...
  ind1plot = d_ki_behandlertilsyn |>
    group_by(SykehusNavn, monthname) |> 
    aggreger_ki_prop()
    
  meanind1plot = d_ki_behandlertilsyn |> 
    group_by(monthname) |> 
    aggreger_ki_prop() |> 
    add_column(SykehusNavn = "Snitt")
  
  #Merging the two tibbles
  ind1plot <- bind_rows(ind1plot, meanind1plot)
  
  #Plotting plotdat
  ggplot(ind1plot, aes(monthname, est)) + 
    geom_line(data = ind1plot |> filter(SykehusNavn != "Snitt"), 
              aes(group = factor(SykehusNavn), colour=factor(SykehusNavn)), size = 0.5) +
    geom_point(data = ind1plot |> filter(SykehusNavn != "Snitt"), 
               aes(group = factor(SykehusNavn), colour=factor(SykehusNavn)), size = 1) + 
    geom_line(data = ind1plot |> filter(SykehusNavn == "Snitt"), 
              aes(group = SykehusNavn, colour=SykehusNavn), size = 1.5) +
    geom_point(data = ind1plot |> filter(SykehusNavn == "Snitt"), 
               aes(group = SykehusNavn, colour=SykehusNavn), size = 1.3) + 
...
#Sterkeste smerte
 tab41 <- mst(
    tab = ki_sme_sterk |> 
      select(SykehusNavn, ki_teller, ki_nevner, est),
    col_names = c("Sykehus", "Antall", "Totalt antall", "Andel"),
    type = params$tableFormat,
    cap = paste0("Oversikt over andel pasienter som hadde nedang i NRS fra første til siste måling for sterkeste smerte."),
    digs = 2,
    align = c("l", "r", "r", "r"))
...
#Svakeste smerte
 tab42 <- mst(
    ...
    col_names = c("Sykehus", "Antall", "Totalt antall", "Andel"),
    type = params$tableFormat,
    cap = paste0("Oversikt over andel pasienter som hadde nedang i NRS fra første til siste måling for svakeste smerte."),
    digs = 2,
    align = c("l", "r", "r", "r"))
}
```
...
if (dim(dat)[1] < 1) {
  ...
  ind41plot = ki_smerteendring_sterkeste |> 
    filter(ki_krit_nevner) |> 
    group_by(SykehusNavn, monthname) |> 
    aggreger_ki_prop() 
  
  meanind41plot = ki_smerteendring_sterkeste |> 
    filter(ki_krit_nevner) |> 
    group_by(monthname) |> 
    aggreger_ki_prop() |> 
    mutate(SykehusNavn = "Snitt")
  
#Merging the two tibbles
  ind41plot <- bind_rows(ind41plot, meanind41plot)
  
  p1 = ggplot(ind41plot, aes(monthname, est)) + 
    plotdetails +
    labs(
      x = "Måned", y = "Andel",
      title = "Andel med nedgang i NRS for sterkeste smerte (månedlig)",
      color = "Sykehus"
    ) 
  
  ind42plot = ki_smerteendring_svakeste |> 
    filter(ki_krit_nevner) |> 
    group_by(SykehusNavn, monthname) |> 
    aggreger_ki_prop()

  meanind42plot = ki_smerteendring_svakeste |> 
    filter(ki_krit_nevner) |> 
    group_by(monthname) |> 
    aggreger_ki_prop() |> 
    mutate(SykehusNavn = "Snitt")
  ...
  #Plotting plotdat
  p2 = ggplot(ind42plot, aes(monthname, est)) + 
    plotdetails +
    labs(
      x = "Måned",
      y = "Andel",
      title = "Andel med nedgang i NRS for svakeste smerte (månedlig)",
      color = "Sykehus"
    )
}
```
...
if (dim(dat)[1] < 1) {
  ...
  #VidereOppf, BehNedtrappAvsluttTils og Journalnotat
  nedtr <- dat |>
    filter(!is.na(AkseptabelSmerte21), Tilsett == 1) |>
