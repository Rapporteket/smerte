...
datmed <- tibble::as_tibble(
  datspinal %>%
    ...
    dplyr::mutate(Opioidtkst = dplyr::case_when(Opbehd221d == 0 ~ "Nei",
                                         Opbehd221d == 1 ~ "Ja",
                                         TRUE ~ "Ikke aktuelt")) %>%
    dplyr::mutate(LAbehtkst = dplyr::case_when(LAbehd221d == 0 ~ "Nei",
                                         LAbehd221d == 1 ~ "Ja",
                                         TRUE ~ "Ikke aktuelt")) %>%
    dplyr::mutate(KoAtkst = dplyr::case_when(KoAbedel221d == 0 ~ "Nei",
                                         KoAbedel221d == 1 ~ "Ja",
                                         TRUE ~ "Ikke aktuelt"))
  )
...
#   cap = paste0("Vedrørende medikament brukt i spinalkateter for alle forløp registert med dette registert med dette. 'MoEkv før' og 'MoEkv etter' gir antall morfinekvivalenter henholdsvis ved innleggelse og ved utskrivelse."),
#   label = "medtab",
#   type = params$tableFormat,
#   digs = 0,
