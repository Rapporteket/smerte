...
mstt <- function(tab, colnames = colnames(tab), type, cap,
                label = opts_current$get("label"), digs, align = NULL,
                fs = 8, lsd = FALSE) {
  if (type == "latex") {
    if (lsd) {
    ...
    k <- kable(tab, col.names = colnames, caption = cap,
...
datbeh = datbeh %>%
  group_by(tilsynstype, ForlopsID) %>% 
...
# dat$stbe = dat %>% mutate(stbe = ifelse(is.na(stbe), "Mangler", stbe))
# dat$svbe = dat %>% mutate(svbe = ifelse(is.na(svbe), "Mangler", svbe))
# dat$stro = dat %>% mutate(stro = ifelse(is.na(stro), "Mangler", stro))
# dat$svro = dat %>% mutate(svro = ifelse(is.na(svro), "Mangler", svro))
...
datmed <- as_tibble(
  datspinal %>%
    select(ForlopsID, MoEkvivalens, MoEkvivalens22, Opbehd221d, 
                  LAbehd221d, KoAbedel221d) %>%
    mutate(Opbehd221d = case_when(Opbehd221d == 0 ~ "Nei",
                                         Opbehd221d == 1 ~ "Ja",
                                         TRUE ~ "Ikke aktuelt")) %>%
    mutate(LAbehd221d = case_when(LAbehd221d == 0 ~ "Nei",
                                         LAbehd221d == 1 ~ "Ja",
                                         TRUE ~ "Ikke aktuelt")) %>%
    mutate(KoAbedel221d = case_when(KoAbedel221d == 0 ~ "Nei",
                                         KoAbedel221d == 1 ~ "Ja",
                                         TRUE ~ "Ikke aktuelt"))
)
...
nrsbev = as_tibble(
  datspinal %>%
    select(ForlopsID, StSmBev12, StSmBev21, SvSmBev12, SvSmBev21)
)
...
nrsro = as_tibble(
  datspinal %>%
    select(ForlopsID, StSmRo12, StSmRo21, SvSmRo12, SvSmRo21)
)
