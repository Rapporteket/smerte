...
underdiag = fulldat %>% 
  ...
  dplyr::mutate(
    DiagKat = dplyr::recode(
      DiagKat,
      !!!smerte::getSmerteDiagKatValueLab(params$registryName, smerteKat = 1)
    )
  )
...
```{r barplot, eval=dataPresent, results='asis', fig.pos= "H", out.extra = '', fig.align='center', out.width='100%'}
...
Videre er pasientene delt inn i følgende smertekategorier:
...
Fordelingen mellom tid til død og de ulike smertekategoriene finner vi både i tabellen og figuren under:
```{r barplot2, eval=dataPresent, results='asis', fig.pos= "H", out.extra = '', fig.align='center', out.width='100%'}
